...
/** Parameters of a DecisionTreeClassifier model. */
...
/** Serialized shape of the kml DecisionTreeClassifier (official serializer). */
type SerializedTree = ReturnType<
  InstanceType<typeof kml.Tree.DecisionTreeClassifier>["toJSON"]
>;

/**
 * DecisionTreeClassifier: interpretable if/else rules for classification.
 * Same JSON-first API as the linear family, but exposes `featureImportances`
 * instead of `coef` / `intercept`.
 */
class DecisionTreeClassifier {
  private model: InstanceType<typeof kml.Tree.DecisionTreeClassifier>;
  ...
  constructor(params?: DecisionTreeClassifierParams) {
    this.model = new kml.Tree.DecisionTreeClassifier(
      params as ConstructorParameters<
        typeof kml.Tree.DecisionTreeClassifier
      >[0],
    );
  }
  ...
  async load(name: string): Promise<void> {
    ...
    this.model = loadModel<
      InstanceType<typeof kml.Tree.DecisionTreeClassifier>
    >(payload.model);
    this.fitted = true;
