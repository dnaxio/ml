...
/** Parameters of a DecisionTreeRegressor model. */
...
/** Serialized shape of the kml DecisionTreeRegressor (official serializer). */
type SerializedTree = ReturnType<
  InstanceType<typeof kml.Tree.DecisionTreeRegressor>["toJSON"]
>;

/**
 * DecisionTreeRegressor: interpretable if/else rules for regression.
 * Same JSON-first API as the linear family, but exposes `featureImportances`
 * instead of `coef` / `intercept`.
 */
class DecisionTreeRegressor {
  private model: InstanceType<typeof kml.Tree.DecisionTreeRegressor>;
   ...
   */
  constructor(params?: DecisionTreeRegressorParams) {
    this.model = new kml.Tree.DecisionTreeRegressor(
      params as ConstructorParameters<typeof kml.Tree.DecisionTreeRegressor>[0],
    );
  }
  ...
  async load(name: string): Promise<void> {
    ...
    this.model = loadModel<InstanceType<typeof kml.Tree.DecisionTreeRegressor>>(
      payload.model,
    );
    this.fitted = true;
