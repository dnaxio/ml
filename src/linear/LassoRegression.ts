...
/** Parameters of a LassoRegression model (constructor config + restore). */
export interface LassoParams {
  /** L1 penalty strength. Default: 1. Higher → more coefficients set to 0. */
  alpha?: number;
...
type LassoLearnedParams = Pick<LassoParams, "coef" | "intercept">;

/**
 * Lasso regression: linear regression with L1 regularization, which can
 * shrink some coefficients to exactly 0 (feature selection). Same JSON-first
 * API as LinearRegression / RidgeRegression.
 */
class LassoRegression {
  private model: InstanceType<typeof kml.Linear.LassoRegression>;
  private transformer: JsonTransformer | null = null;
  private fitted = false;

  /**
   * Creates a model. `alpha` / `fitIntercept` / `maxIter` / `tol` configure
   * the regularization; `coef` / `intercept` optionally restore a trained
   * model.
   * @param params - Model parameters.
   */
  constructor(params?: LassoParams) {
    const { coef, intercept, alpha, fitIntercept, maxIter, tol } = params ?? {};
    this.model = new kml.Linear.LassoRegression({
      alpha,
      fitIntercept,
      maxIter,
      tol,
   ...
   */
  setParams(params: LassoLearnedParams): this {
    const allowed: (keyof LassoLearnedParams)[] = ["coef", "intercept"];
    for (const key of Object.keys(params) as (keyof LassoLearnedParams)[]) {
   ...
   */
  getParams(): Required<LassoLearnedParams> {
  ...
  async export(name: string): Promise<void> {
    ...
    const m = configOf(this.model, ["alpha", "fitIntercept", "maxIter", "tol"]);
    const payload = {
