...
/** Parameters of an XGBoostClassifier model. */
export interface XGBoostClassifierParams {
...
/** Serialized shape of the kml XGBoostClassifier (official serializer). */
type SerializedXGB = ReturnType<
  InstanceType<typeof kml.Ensemble.XGBoostClassifier>["toJSON"]
>;

/**
 * XGBoostClassifier: regularized, exact-greedy gradient boosting close to
 * the xgboost library. Binary (logistic) and multiclass (softmax) support.
 * Exposes calibrated `predict_proba` / `fill_predict_proba` and
 * `featureImportances`.
 */
class XGBoostClassifier {
  private model: InstanceType<typeof kml.Ensemble.XGBoostClassifier>;
   ...
   */
  constructor(params?: XGBoostClassifierParams) {
    this.model = new kml.Ensemble.XGBoostClassifier(
      params as ConstructorParameters<typeof kml.Ensemble.XGBoostClassifier>[0],
    );
  }
  ...
  /**
   * Predicts calibrated per-class probabilities on JSON rows.
   * @param data - Row objects with the same features fields as at fit time.
  ...
  async load(name: string): Promise<void> {
    ...
    this.model = loadModel<InstanceType<typeof kml.Ensemble.XGBoostClassifier>>(
      payload.model,
    );
    this.fitted = true;
