...
/** Parameters of a GradientBoostingRegressor model. */
export interface GradientBoostingRegressorParams {
...
/** Serialized shape of the kml GradientBoostingRegressor (official serializer). */
type SerializedGBR = ReturnType<
  InstanceType<typeof kml.Ensemble.GradientBoostingRegressor>["toJSON"]
>;

/**
 * GradientBoostingRegressor: additive chain of shallow trees fitted to the
 * residuals of the previous trees (gradient descent on squared error).
 * A strong default for tabular regression. Exposes `featureImportances`
 * instead of `coef` / `intercept`.
 */
class GradientBoostingRegressor {
  private model: InstanceType<typeof kml.Ensemble.GradientBoostingRegressor>;
   ...
   */
  constructor(params?: GradientBoostingRegressorParams) {
    this.model = new kml.Ensemble.GradientBoostingRegressor(
      params as ConstructorParameters<
        typeof kml.Ensemble.GradientBoostingRegressor
      >[0],
    );
  }
  ...
  async load(name: string): Promise<void> {
    ...
    this.model = loadModel<
      InstanceType<typeof kml.Ensemble.GradientBoostingRegressor>
    >(payload.model);
    this.fitted = true;
