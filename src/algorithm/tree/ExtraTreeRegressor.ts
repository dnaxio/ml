...
/** Parameters of an ExtraTreeRegressor model. */
export interface ExtraTreeRegressorParams {
...
/** Serialized shape of the kml ExtraTreeRegressor (official serializer). */
type SerializedTree = ReturnType<
  InstanceType<typeof kml.Tree.ExtraTreeRegressor>["toJSON"]
>;

/**
 * ExtraTreeRegressor: randomized decision tree regressor. More random
 * splits than DecisionTreeRegressor, usually a better baseline.
 */
class ExtraTreeRegressor {
  private model: InstanceType<typeof kml.Tree.ExtraTreeRegressor>;
   ...
   */
  constructor(params?: ExtraTreeRegressorParams) {
    this.model = new kml.Tree.ExtraTreeRegressor(
      params as ConstructorParameters<typeof kml.Tree.ExtraTreeRegressor>[0],
    );
  }
  ...
  async load(name: string): Promise<void> {
    ...
    this.model = loadModel<InstanceType<typeof kml.Tree.ExtraTreeRegressor>>(
      payload.model,
    );
    this.fitted = true;
