...
/** Parameters of an AdaBoostClassifier model. */
export interface AdaBoostClassifierParams {
  /** Number of boosting iterations (stumps). Default: 50. */
  nEstimators?: number;
  /** Weight applied to each stump. Default: 1.0. */
  learningRate?: number;
  /** Seed for reproducible training. */
  randomState?: number;
}

/** Serialized shape of the kml AdaBoostClassifier (official serializer). */
type SerializedAda = ReturnType<
  InstanceType<typeof kml.Ensemble.AdaBoostClassifier>["toJSON"]
>;

/**
 * AdaBoostClassifier: sequential boosting over decision stumps, each one
 * focusing on the samples misclassified by the previous rounds. Exposes
 * `predict_proba` / `fill_predict_proba` (heuristic per-class probabilities)
 * and `featureImportances`.
 */
class AdaBoostClassifier {
  private model: InstanceType<typeof kml.Ensemble.AdaBoostClassifier>;
   ...
   */
  constructor(params?: AdaBoostClassifierParams) {
    this.model = new kml.Ensemble.AdaBoostClassifier(
      params as ConstructorParameters<
        typeof kml.Ensemble.AdaBoostClassifier
      >[0],
    );
  }
  ...
  async load(name: string): Promise<void> {
    ...
    this.model = loadModel<
      InstanceType<typeof kml.Ensemble.AdaBoostClassifier>
    >(payload.model);
    this.fitted = true;
